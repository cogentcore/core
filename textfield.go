...
type TextField struct {
	...
	completer       Completer               `json:"-" xml:"-" desc:"current text completer"`
	completeFunc    CompletionFunc          `desc:"callback to get the list of possible matches if not generated by the completer itself"`
	completeContext interface{}             `desc:"the caller"`
...
func (tf *TextField) OfferCompletions() {
	...
	tf.completer = nil // start fresh
	if !tf.enableComplete {
		return
	}

	// create completer and populate menu
	tf.completer = new(BasicCompleter)
	var list []string
	var seed string
	if tf.completeFunc != nil {
		list, seed = tf.completeFunc(string(tf.EditTxt[0:tf.CursorPos]))
	} else {
		seed = SeedWhiteSpace(string(tf.EditTxt[0:tf.CursorPos]))
	}
	tf.completer.Matches(list, seed)
	if tf.completer.Count() == 1 { // don't show if only one and it matches current text
		if tf.completer.Match(0) == tf.completer.Seed() {
			return
		}
	}
	if tf.completer.Count() > 0 {
		m := tf.MakeCompletionMenu(tf.completer)
		cpos := tf.CharStartPos(tf.CursorPos).ToPoint()
...
func (tf *TextField) Complete(str string) {
	...
	s1 = strings.TrimSuffix(s1, tf.completer.Seed())
	s1 += str
	txt := s1 + s2
	tf.EditTxt = []rune(txt)
	tf.CursorForward(len(str) - len(tf.completer.Seed()))
	tf.completer = nil
}
...
func (tf *TextField) KeyInput(kt *key.ChordEvent) {
	...
	if PopupIsCompleter(tf.ParentWindow().Popup) {
		...
		case KeyFunFocusNext: // tab will - complete if single item or try to extend if multiple
			if tf.completer.Count() == 1 {
				tf.Complete(tf.completer.Match(0))
				tf.ParentWindow().ClosePopup(tf.ParentWindow().Popup)
				return
			}
			// try to extend the seed
			s := tf.completer.Extend()
			if s != "" {
...
func (tf *TextField) SetCompleter(data interface{}, fun CompletionFunc) {
