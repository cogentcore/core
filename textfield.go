...
type TextField struct {
	...
	OurCompleter    Completer               `json:"-" xml:"-" desc:"current text completer"`
...
func (tf *TextField) Complete(str string) {
	...
	s1 = strings.TrimSuffix(s1, tf.OurCompleter.GetSeed())
	s1 += str
	txt := s1 + s2
	tf.EditTxt = []rune(txt)
	tf.CursorForward(len(str) - len(tf.OurCompleter.GetSeed()))
	tf.OurCompleter = nil
}
...
func (tf *TextField) KeyInput(kt *key.ChordEvent) {
	...
	if PopupIsCompleter(tf.ParentWindow().Popup) {
		...
		case KeyFunFocusNext: // tab will - complete if single item or try to extend if multiple
			if tf.OurCompleter.Count() == 1 {
				tf.Complete(tf.OurCompleter.Match(0))
				tf.ParentWindow().ClosePopup(tf.ParentWindow().Popup)
				return
			}
			// try to extend the seed
			s := tf.OurCompleter.Extend()
			if s != "" {
