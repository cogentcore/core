...
// Init initializes the given node. If a parent is given, it also
// sets the parent of the node to that parent. It must be called on
// all nodes. It is automatically called by the autogenerated
// New functions in typegen, so most end users will not need to call it.
func Init[T Node](n T, parent ...Node) T {
	initNode(n)
	if len(parent) > 0 {
		parent[0].AddChild(n)
	}
...
// initNode initializes the given node.
...
func checkThis(n Node) error {
	...
	return errors.Log(fmt.Errorf("tree.Node %q has nil Node.This; you must use the autogenerated New functions or call tree.Init", n.Path()))
}
