...
func (mt *Material) ApplyCSS(node Node, css ki.Props, key, selector string, vp *gi.Viewport2D) bool {
	pp, got := css[key]
	if !got {
		return false
	}
	...
	if selector != "" {
		pmap, ok = styles.SubProps(pmap, selector)
		if !ok {
			return false
		}
	}
...
// (:hover, :active etc)
func (mt *Material) StyleCSS(node Node, css ki.Props, selector string, vp *gi.Viewport2D) {
...
var StyleMatFuncs = map[string]styles.StyleFunc{
	"color": func(obj any, key string, val any, par any, ctxt styles.Context) {
		mt := obj.(*Material)
		if inh, init := styles.StyleInhInit(val, par); inh || init {
			if inh {
	...
	"emissive": func(obj any, key string, val any, par any, ctxt styles.Context) {
		mt := obj.(*Material)
		if inh, init := styles.StyleInhInit(val, par); inh || init {
			if inh {
	...
	"shiny": func(obj any, key string, val any, par any, ctxt styles.Context) {
		mt := obj.(*Material)
		if inh, init := styles.StyleInhInit(val, par); inh || init {
			if inh {
	...
	"reflective": func(obj any, key string, val any, par any, ctxt styles.Context) {
		mt := obj.(*Material)
		if inh, init := styles.StyleInhInit(val, par); inh || init {
			if inh {
	...
	"bright": func(obj any, key string, val any, par any, ctxt styles.Context) {
		mt := obj.(*Material)
		if inh, init := styles.StyleInhInit(val, par); inh || init {
			if inh {
	...
	"texture": func(obj any, key string, val any, par any, ctxt styles.Context) {
		mt := obj.(*Material)
		if inh, init := styles.StyleInhInit(val, par); inh || init {
			if inh {
	...
	"cull-back": func(obj any, key string, val any, par any, ctxt styles.Context) {
		mt := obj.(*Material)
		if inh, init := styles.StyleInhInit(val, par); inh || init {
			if inh {
	...
	"cull-front": func(obj any, key string, val any, par any, ctxt styles.Context) {
		mt := obj.(*Material)
		if inh, init := styles.StyleInhInit(val, par); inh || init {
			if inh {
