...
var _KindsNameToValueMap = map[string]Kinds{
	...
	`Uint64`:    15,
	`uint64`:    15,
	`generic raw pointer data value -- see also Ptr, Ref for more semantic cases`: 16,
	`generic raw pointer data value -- see also ptr, ref for more semantic cases`: 16,
	`pointer -- element is what we point to (kind of a composite type)`:           17,
	`reference -- element is what we refer to`:                                    18,
	`for case where these are distinguished from Ptr (Go) -- similar to Uintptr`:  19,
	`for case where these are distinguished from ptr (go) -- similar to uintptr`:  19,
	`Fixed`:      20,
	`fixed`:      20,
	`Fixed26_6`:  21,
	...
	`Tuple`:      33,
	`tuple`:      33,
	`a special kind of tuple for Python ranges`: 34,
	`a special kind of tuple for python ranges`: 34,
	`Array`: 35,
	`array`: 35,
	`List`:  36,
	`list`:  36,
	`List of some type of char rep -- Type.Els is type, as all Lists`: 37,
	`list of some type of char rep -- type.els is type, as all lists`: 37,
	`Matrix`:                              38,
	`matrix`:                              38,
	`Tensor`:                              39,
	...
	`Set`:                                 41,
	`set`:                                 41,
	`python's frozen set of fixed values`: 42,
	`Struct`:                              43,
	`struct`:                              43,
	`Class`:                               44,
	`class`:                               44,
...
var _KindsMap = map[Kinds]string{
	...
	14: `Uint32`,
	15: `Uint64`,
	16: `generic raw pointer data value -- see also Ptr, Ref for more semantic cases`,
	17: `pointer -- element is what we point to (kind of a composite type)`,
	18: `reference -- element is what we refer to`,
	19: `for case where these are distinguished from Ptr (Go) -- similar to Uintptr`,
	20: `Fixed`,
	21: `Fixed26_6`,
	22: `Fixed16_6`,
	23: `Fixed0_32`,
	...
	32: `Composite`,
	33: `Tuple`,
	34: `a special kind of tuple for Python ranges`,
	35: `Array`,
	36: `List`,
	37: `List of some type of char rep -- Type.Els is type, as all Lists`,
	38: `Matrix`,
	39: `Tensor`,
	40: `Map`,
	41: `Set`,
	42: `python's frozen set of fixed values`,
	43: `Struct`,
	44: `Class`,
	45: `Object`,
	46: `Chan`,
