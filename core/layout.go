...
func (fr *Frame) laySetContentFitOverflow(nsz math32.Vector2, pass LayoutPasses) {
	...
	*isz = nsz                   // internal is always accurate!
	...
	for d := math32.X; d <= math32.Y; d++ {
		...
		if !(fr.Scene != nil && fr.Scene.hasFlag(sceneContentsSizing)) && oflow.Dim(d) >= styles.OverflowAuto && fr.Parent != nil {
			if mx.Dim(d) > 0 {
...
func (wb *WidgetBase) growToAlloc() bool {
	if (wb.Scene != nil && wb.Scene.hasFlag(sceneContentsSizing)) || wb.Styles.GrowWrap {
		return false
	}
	sz := &wb.Geom.Size
