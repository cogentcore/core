...
func intersectionSegment(zs Intersections, a0 math32.Vector2, a []float32, b0 math32.Vector2, b []float32) Intersections {
	n := len(zs)
	swapCurves := false
	if a[0] == LineToCmd || a[0] == CloseCmd {
		if b[0] == LineToCmd || b[0] == CloseCmd {
			zs = intersectionLineLine(zs, a0, math32.Vector2{a[1], a[2]}, b0, math32.Vector2{b[1], b[2]})
		} else if b[0] == QuadToCmd {
			zs = intersectionLineQuad(zs, a0, math32.Vector2{a[1], a[2]}, b0, math32.Vector2{b[1], b[2]}, math32.Vector2{b[3], b[4]})
		} else if b[0] == CubeToCmd {
			zs = intersectionLineCube(zs, a0, math32.Vector2{a[1], a[2]}, b0, math32.Vector2{b[1], b[2]}, math32.Vector2{b[3], b[4]}, math32.Vector2{b[5], b[6]})
			...
			zs = intersectionLineEllipse(zs, a0, math32.Vector2{a[1], a[2]}, math32.Vector2{cx, cy}, math32.Vector2{rx, ry}, phi, theta0, theta1)
		}
	} else if a[0] == QuadToCmd {
		if b[0] == LineToCmd || b[0] == CloseCmd {
			zs = intersectionLineQuad(zs, b0, math32.Vector2{b[1], b[2]}, a0, math32.Vector2{a[1], a[2]}, math32.Vector2{a[3], a[4]})
			swapCurves = true
		} else if b[0] == QuadToCmd {
		...
		if b[0] == LineToCmd || b[0] == CloseCmd {
			zs = intersectionLineCube(zs, b0, math32.Vector2{b[1], b[2]}, a0, math32.Vector2{a[1], a[2]}, math32.Vector2{a[3], a[4]}, math32.Vector2{a[5], a[6]})
			swapCurves = true
		} else if b[0] == QuadToCmd {
		...
		if b[0] == LineToCmd || b[0] == CloseCmd {
			zs = intersectionLineEllipse(zs, b0, math32.Vector2{b[1], b[2]}, math32.Vector2{cx, cy}, math32.Vector2{rx, ry}, phi, theta0, theta1)
			swapCurves = true
		} else if b[0] == QuadToCmd {
...
func (z Intersection) Equals(o Intersection) bool {
	return z.math32.Vector2.Equals(o.math32.Vector2) && Equal(z.T[0], o.T[0]) && Equal(z.T[1], o.T[1]) && angleEqual(z.Dir[0], o.Dir[0]) && angleEqual(z.Dir[1], o.Dir[1]) && z.Tangent == o.Tangent && z.Same == o.Same
}
...
func (z Intersection) String() string {
	...
	return fmt.Sprintf("({%v,%v} t={%v,%v} dir={%v°,%v°}%v)", numEps(z.math32.Vector2.X), numEps(z.math32.Vector2.Y), numEps(z.T[0]), numEps(z.T[1]), numEps(angleNorm(z.Dir[0])*180.0/math.Pi), numEps(angleNorm(z.Dir[1])*180.0/math.Pi), extra)
}
...
func intersectionEllipseEllipse(zs Intersections, c0, r0 math32.Vector2, phi0, thetaStart0, thetaEnd0 float32, c1, r1 math32.Vector2, phi1, thetaStart1, thetaEnd1 float32) Intersections {
	...
	dev := math32.Vector2{c1.Y - c0.Y, c0.X - c1.X}.Mul(c)

	tangent := dev.Equals(math32.Vector2{})
...
func intersectionCircleCircle(c0 math32.Vector2, r0 float32, c1 math32.Vector2, r1 float32) (math32.Vector2, math32.Vector2, bool) {
	...
	i0 := c0.Add(c1).Mul(a)
	i1 := c1.Sub(c0).Mul(b)
	i2 := math32.Vector2{c1.Y - c0.Y, c0.X - c1.X}.Mul(c)
