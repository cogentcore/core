...
var _LayoutFlagsValues = []LayoutFlags{10, 11, 12, 13}

// LayoutFlagsN is the highest valid value
// for type LayoutFlags, plus one.
const LayoutFlagsN LayoutFlags = 14

// An "invalid array index" compiler error signifies that the constant values have changed.
// Re-run the enumgen command to generate them again.
func _LayoutFlagsNoOp() {
	var x [1]struct{}
	_ = x[LayoutStackTopOnly-(10)]
	_ = x[LayoutNeedsRedo-(11)]
	_ = x[LayoutScrollsOff-(12)]
	_ = x[LayoutNoKeys-(13)]
}
...
var _LayoutFlagsDescMap = map[LayoutFlags]string{
	10: `for stacked layout, only layout the top widget. this is appropriate for e.g., tab layout, which does a full redraw on stack changes, but not for e.g., check boxes which don&#39;t`,
	11: `true if this layout got a redo = true on previous iteration -- otherwise it just skips any re-layout on subsequent iteration`,
	12: `scrollbars have been manually turned off due to layout being invisible -- must be reactivated when re-visible`,
	13: `LayoutNoKeys prevents processing of keyboard events for this layout. By default, Layout handles focus navigation events, but if an outer Widget handles these instead, then this should be set.`,
}
