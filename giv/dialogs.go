...
func TextViewDialog(ctx gi.Widget, opts DlgOpts, text []byte, fun func(dlg *gi.Dialog)) *textview.TextView {
	var dlg *gi.Dialog
	if opts.Data != nil {
		recyc := false
	...
	tv := textview.NewTextView(tlv, "text-view")
...
func TextViewDialogTextView(dlg *gi.Dialog) *textview.TextView {
	frame := dlg.Stage.Scene
	tlv := frame.ChildByName("text-lay", 2)
	tv := tlv.ChildByName("text-view", 0)
	return tv.(*textview.TextView)
}
