...
func (cv *ColorMapView) ChooseColorMap() {
	...
	si := 0
	d := gi.NewDialog(cv).Title("Select a color map").Prompt("Choose color map to use from among available list")
...
func (vv *ColorMapValue) OpenDialog(ctx gi.Widget, fun func(dlg *gi.Dialog)) {
	...
	si := 0
	d := gi.NewDialog(ctx).Title("Select a color map").Prompt(vv.Doc())
	NewSliceView(d).SetSlice(&sl).SetSelVal(cur).BindSelectDialog(d, &si)
	d.OnAccept(func(e events.Event) {
		if si >= 0 {
			vv.SetValue(sl[si])
