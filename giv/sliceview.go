...
func (sv *SliceView) StyleRow(w gi.Widget, index, fidx int) {
	if sv.StyleFunc != nil {
		sv.StyleFunc(w, &w.AsWidget().Styles, index)
	}
}
...
type SliceViewer interface {
	...
	StyleRow(w gi.Widget, index, fidx int)
	...
	PasteAssign(md mimedata.Mimes, index int)

	// PasteAtIdx inserts object(s) from mime data at
	// (before) given slice index
	PasteAtIdx(md mimedata.Mimes, index int)

	MakePasteMenu(m *gi.Scene, md mimedata.Mimes, index int, mod events.DropMods, fun func())
...
// means the end
func (sv *SliceViewBase) SliceNewAt(index int) {
	...
	if iski && sv.SliceValView != nil {
				...
				d.AddBottomBar(func(pw gi.Widget) {
					...
					d.AddOk(pw).OnClick(func(e events.Event) {
						...
						for i := 0; i < nd.Number; i++ {
							nm := fmt.Sprintf("New%v%v", nd.Type.Name, index+1+i)
							ownki.InsertNewChild(nd.Type, index+1+i, nm)
						}
						sv.SetChanged()
		...
		if index >= 0 && index < sz {
			reflect.Copy(svnp.Slice(index+1, sz+1), svnp.Slice(index, sz))
			svnp.Index(index).Set(nval)
		}
		svl.Elem().Set(svnp)
	}
	if index < 0 {
...
func (sv *SliceViewBase) SliceNewAtSel(index int) {
...
func (sv *SliceViewBase) SliceDeleteAtSel(index int) {
...
// SliceDeleteAt deletes element at given index from slice
func (sv *SliceViewBase) SliceDeleteAt(index int) {
	if sv.Is(SliceViewIsArray) {
		return
	}
	if index < 0 || index >= sv.SliceSize {
		return
	}
	sv.ViewMuLock()
	updt := sv.UpdateStart()
	...
	sv.SliceDeleteAtSel(index)

	laser.SliceDeleteAt(sv.Slice, index)
...
func (sv *SliceViewBase) SliceVal(index int) any {
	if index < 0 || index >= sv.SliceSize {
		fmt.Printf("giv.SliceViewBase: slice index out of range: %v\n", index)
		return nil
	}
	val := laser.OnePtrUnderlyingValue(sv.SliceNPVal.Index(index)) // deal with pointer lists
	vali := val.Interface()
	return vali
}
...
func (sv *SliceViewBase) IsIdxVisible(index int) bool {
	return sv.IsRowInBounds(index - sv.StartIdx)
}
...
func (sv *SliceViewBase) IdxGrabFocus(index int) *gi.WidgetBase {
	sv.ScrollToIdx(index)
	return sv.This().(SliceViewer).RowGrabFocus(index - sv.StartIdx)
}

// IdxPos returns center of window position of index label for idx (ContextMenuPos)
func (sv *SliceViewBase) IdxPos(index int) image.Point {
	row := index - sv.StartIdx
	if row < 0 {
		row = 0
	}
	if row > sv.VisRows-1 {
...
func (sv *SliceViewBase) ScrollToIdxNoUpdt(index int) bool {
	if sv.VisRows == 0 {
		return false
	}
	if index < sv.StartIdx {
		sv.StartIdx = index
		sv.StartIdx = max(0, sv.StartIdx)
		sv.UpdateScroll()
		return true
	}
	if index >= sv.StartIdx+sv.VisRows {
		sv.StartIdx = index - (sv.VisRows - 4)
		sv.StartIdx = max(0, sv.StartIdx)
		sv.UpdateScroll()
		return true
	}
...
func (sv *SliceViewBase) ScrollToEnd() bool {
	return sv.ScrollToIdx(0) //todo
}

// ScrollToIdx ensures that given slice idx is visible
// by scrolling display as needed.
func (sv *SliceViewBase) ScrollToIdx(index int) bool {
	updt := sv.ScrollToIdxNoUpdt(index)
	if updt {
		if sv.This() == nil {
			return false //???
		}
...
func (sv *SliceViewBase) SelectIdxWidgets(index int, sel bool) bool {
	if !sv.IsIdxVisible(index) {
		return false
	}
	sv.SelectRowWidgets(index-sv.StartIdx, sel)
	return true
}
...
func (sv *SliceViewBase) UpdateSelectIdx(index int, sel bool, selMode events.SelectModes) {
	if sv.IsReadOnly() && !sv.Is(SliceViewReadOnlyMultiSel) {
		updt := sv.UpdateStart()
		defer sv.UpdateEndRender(updt)

		sv.UnselectAllIdxs()
		if sel || sv.SelIdx == index {
			sv.SelIdx = index
			sv.SelectIdx(index)
		}
		sv.ApplyStyleTree()
		...
		sv.SelectIdxAction(index, selMode)
	}
}

// IdxIsSelected returns the selected status of given slice index
func (sv *SliceViewBase) IdxIsSelected(index int) bool {
	sv.ViewMuLock()
	defer sv.ViewMuUnlock()
	if sv.IsReadOnly() {
		return index == sv.SelIdx
	}
	_, ok := sv.SelIdxs[index]
	return ok
}
...
func (sv *SliceViewBase) SelectIdx(index int) {
	sv.SelIdxs[index] = struct{}{}
	// sv.SelectIdxWidgets(idx, true)
}

// UnselectIdx unselects given idx (if selected)
func (sv *SliceViewBase) UnselectIdx(index int) {
	if sv.IdxIsSelected(index) {
		delete(sv.SelIdxs, index)
	}
...
func (sv *SliceViewBase) SelectIdxAction(index int, mode events.SelectModes) {
	if mode == events.NoSelect {
		return
	}
	index = min(index, sv.SliceSize-1)
	if index < 0 {
		sv.ResetSelectedIdxs()
		return
	}
	// row := index - sv.StartIdx // note: could be out of bounds
	updt := sv.UpdateStart()
	defer sv.UpdateEndRender(updt)
...
func (sv *SliceViewBase) UnselectIdxAction(index int) {
	if sv.IdxIsSelected(index) {
		sv.UnselectIdx(index)
	}
}
...
func (sv *SliceViewBase) MimeDataIdx(md *mimedata.Mimes, index int) {
	sv.ViewMuLock()
	val := sv.SliceVal(index)
...
func (sv *SliceViewBase) PasteIdx(index int) { //gti:add
	sv.TmpIdx = index
...
func (sv *SliceViewBase) MakePasteMenu(m *gi.Scene, md mimedata.Mimes, index int, mod events.DropMods, fun func()) {
	svi := sv.This().(SliceViewer)
	if mod == events.DropCopy {
		gi.NewButton(m).SetText("Assign to").OnClick(func(e events.Event) {
			svi.PasteAssign(md, index)
			if fun != nil {
				fun()
			}
		})
...
func (sv *SliceViewBase) PasteMenu(md mimedata.Mimes, index int) {
	sv.UnselectAllIdxs()
	mf := func(m *gi.Scene) {
		sv.MakePasteMenu(m, md, index, events.DropCopy, nil)
	}
	pos := sv.IdxPos(index)
...
// PasteAssign assigns mime data (only the first one!) to this idx
func (sv *SliceViewBase) PasteAssign(md mimedata.Mimes, index int) {
	...
	updt := sv.UpdateStart()
	ns := sl[0]
	sv.SliceNPVal.Index(index).Set(reflect.ValueOf(ns).Elem())
	if sv.TmpSave != nil {
		sv.TmpSave.SaveTmp()
	}
	sv.SetChanged()
...
func (sv *SliceViewBase) PasteAtIdx(md mimedata.Mimes, index int) {
	sl := sv.FromMimeData(md)
	if len(sl) == 0 {
		return
	}
	...
	for _, ns := range sl {
		...
		if index >= 0 && index < sz {
			reflect.Copy(svnp.Slice(index+1, sz+1), svnp.Slice(index, sz))
			svnp.Index(index).Set(reflect.ValueOf(ns).Elem())
			svl.Elem().Set(svnp)
		}
		index++
	}
	...
	sv.SetChanged()
	sv.SelectIdxAction(index, events.SelectOne)
	sv.This().(SliceViewer).UpdateWidgets()
}
...
func (sv *SliceViewBase) SaveDraggedIdxs(index int) {
	sz := len(sv.SelIdxs)
	if sz == 0 {
		sv.DraggedIdxs = nil
		return
...
func (sg *SliceViewGrid) UpdateScroll(index int) {
	if !sg.HasScroll[mat32.Y] || sg.Scrolls[mat32.Y] == nil {
		return
	}
	sb := sg.Scrolls[mat32.Y]
	sb.SetValue(float32(index))
}
