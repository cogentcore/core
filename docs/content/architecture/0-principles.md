...
This is the approach taken in Cogent Core (along with auto-generated `Set` methods for chaining as discussed below).  In general, setting fields should have no side-effects, and instead all the side-effects should be handled in the `Update()` method (via [[tree.NodeBase]] `Updaters` or [[core.WidgetBase]] `Stylers` as appropriate).
