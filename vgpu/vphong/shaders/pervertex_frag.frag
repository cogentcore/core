#version 450
#extension GL_EXT_nonuniform_qualifier : require

// must be <= 128 bytes -- contains all per-object data
layout(push_constant) uniform PushU {
	mat4 ModelMtx; // 64 bytes, [3][3] = TexPct.X
	vector4 Color; // 16
	vector4 ShinyBright; // 16 x = Shiny, y = Reflect, z = Bright, w = TexIndex
	vector4 Emissive; // 16 rgb, a = TexPct.Y
	vector4 TexRepeatOff; // 16 xy = Repeat, zw = Offset
};

layout(set = 0, binding = 0) uniform MtxsU {
    mat4 ViewMtx;
    mat4 PrjnMtx;
};

layout(location = 0) in vector4 Pos;
layout(location = 1) in vector3 Norm;
layout(location = 2) in vector3 CamDir;
// layout(location = 3) in vector2 TexCoord;
layout(location = 3) in vector4 VtxColor;

layout(location = 0) out vector4 outColor;

#include "phong_frag.frag"
			
void main() {
	float opacity = VtxColor.a;
	vector3 clr = SRGBToLinear(VtxColor.rgb);	// we need to undo gamma on incoming colors
	// vector3 clr = VtxColor.rgb;
	
	// Calculates the Ambient+Diffuse and Specular colors for this fragment using the Phong model.
	float Shiny = ShinyBright.x;
	float Reflect = 0; // ShinyBright.y;
	float Bright = ShinyBright.z;
	vector3 Specular = vector3(1,1,1);
	PhongModel(Pos, Norm, CamDir, clr, clr, Specular, Shiny, Reflect, Bright, opacity, outColor);
}

