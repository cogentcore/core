...
func mainrun() {
	...
	bg := vp.AddNewChildNamed(reflect.TypeOf(gi.GiRect{}), "bg").(*gi.GiRect)
	bg.SetProp("fill", "#FFF")
	bg.Pos = gi.Point2D{0, 00}
	bg.Size = gi.Size2D{float64(width), float64(height)}

	// rect1.SetProp("stroke-linejoin", "round")
	rect1 := bg.AddNewChildNamed(reflect.TypeOf(gi.GiRect{}), "rect1").(*gi.GiRect)
	...
	circle1 := bg.AddNewChildNamed(reflect.TypeOf(gi.GiCircle{}), "circle1").(*gi.GiCircle)
	...
	line1 := bg.AddNewChildNamed(reflect.TypeOf(gi.GiLine{}), "line1").(*gi.GiLine)
	line1.SetProp("stroke", "#888800")
	line1.SetProp("stroke-width", 5.0)
	line1.Start = gi.Point2D{100, 100}
	line1.End = gi.Point2D{150, 200}

	polyline1 := bg.AddNewChildNamed(reflect.TypeOf(gi.GiPolyline{}), "polyline1").(*gi.GiPolyline)
	...
	polygon1 := bg.AddNewChildNamed(reflect.TypeOf(gi.GiPolygon{}), "polygon1").(*gi.GiPolygon)
