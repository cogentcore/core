...
var _ = types.AddType(&types.Type{Name: "cogentcore.org/core/texteditor.Buffer", IDName: "buffer", Doc: "Buffer is a buffer of text, which can be viewed by [Editor](s).\nIt holds the raw text lines (in original string and rune formats,\nand marked-up from syntax highlighting), and sends signals for making\nedits to the text and coordinating those edits across multiple views.\nEditors always only view a single buffer, so they directly call methods\non the buffer to drive updates, which are then broadcast.\nIt also has methods for loading and saving buffers to files.\nUnlike GUI widgets, its methods generally send events, without an\nexplicit Event suffix.\nInternally, the buffer represents new lines using \\n = LF, but saving\nand loading can deal with Windows/DOS CRLF format.", Directives: []types.Directive{{Tool: "types", Directive: "add"}}, Methods: []types.Method{{Name: "Open", Doc: "Open loads the given file into the buffer.", Directives: []types.Directive{{Tool: "types", Directive: "add"}}, Args: []string{"filename"}, Returns: []string{"error"}}, {Name: "Revert", Doc: "Revert re-opens text from the current file, if the filename is set; returns false if\nnot. It uses an optimized diff-based update to preserve existing formatting, making it\nvery fast if not very different.", Directives: []types.Directive{{Tool: "types", Directive: "add"}}, Returns: []string{"bool"}}, {Name: "SaveAs", Doc: "SaveAs saves the current text into given file; does an editDone first to save edits\nand checks for an existing file; if it does exist then prompts to overwrite or not.", Directives: []types.Directive{{Tool: "types", Directive: "add"}}, Args: []string{"filename"}}, {Name: "Save", Doc: "Save saves the current text into the current filename associated with this buffer.", Directives: []types.Directive{{Tool: "types", Directive: "add"}}, Returns: []string{"error"}}}, Fields: []types.Field{{Name: "Filename", Doc: "Filename is the filename of the file that was last loaded or saved.\nIt is used when highlighting code."}, {Name: "text", Doc: "text is the current value of the entire text being edited,\nrepresented as a byte slice for efficiency."}, {Name: "Autosave", Doc: "Autosave specifies whether the file should be automatically\nsaved after changes are made."}, {Name: "Options", Doc: "Options are the options for how text editing and viewing works."}, {Name: "Info", Doc: "Info is the full information about the current file."}, {Name: "ParseState", Doc: "ParseState is the parsing state information for the file."}, {Name: "Highlighting", Doc: "Highlighting is the syntax highlighting markup parameters, such as the language and style."}, {Name: "NumLines", Doc: "NumLines is the number of lines in the buffer."}, {Name: "LineColors", Doc: "LineColors are the colors to use for rendering circles\nnext to the line numbers of certain lines."}, {Name: "Lines", Doc: "Lines are the live lines of text being edited, with the latest modifications.\nThey are encoded as runes per line, which is necessary for one-to-one rune/glyph\nrendering correspondence. All TextPos positions are in rune indexes, not byte\nindexes."}, {Name: "lineBytes", Doc: "lineBytes are the live lines of text being edited, with the latest modifications.\nThey are encoded in bytes per line, translated from Lines, and used for input to\nmarkup. It is essential to use Lines and not lineBytes when dealing with TextPos\npositions, which are in runes."}, {Name: "tags", Doc: "tags are the extra custom tagged regions for each line."}, {Name: "hiTags", Doc: "hiTags are the syntax highlighting tags, which are auto-generated."}, {Name: "Markup", Doc: "Markup is the marked-up version of the edited text lines, after being run\nthrough the syntax highlighting process. This is what is actually rendered."}, {Name: "markupEdits", Doc: "markupEdits are the edits that have been made since the last full markup."}, {Name: "byteOffsets", Doc: "byteOffsets are the offsets for the start of each line in the [Buffer.text] byte\nslice. This is not updated with edits. Call SetByteOffs to set it when needed.\nIt is used for re-generating the text in LinesToBytes and set on initial open in\nBytesToLines."}, {Name: "totalBytes", Doc: "totalBytes is the total number of bytes in the document.\nSee byteOffsets for when it is updated."}, {Name: "LinesMu", Doc: "LinesMu is the mutex for updating lines."}, {Name: "markupMu", Doc: "markupMu is the mutex for updating markup."}, {Name: "markupDelayTimer", Doc: "markupDelayTimer is the markup delay timer."}, {Name: "markupDelayMu", Doc: "markupDelayMu is the mutex for updating the markup delay timer."}, {Name: "editors", Doc: "editors are the editors that are currently viewing this buffer."}, {Name: "Undos", Doc: "Undos is the undo manager."}, {Name: "posHistory", Doc: "posHistory is the history of cursor positions.\nIt can be used to move back through them."}, {Name: "Complete", Doc: "Complete is the functions and data for text completion."}, {Name: "spell", Doc: "spell is the functions and data for spelling correction."}, {Name: "currentEditor", Doc: "currentEditor is the current text editor, such as the one that initiated the\nComplete or Correct process. The cursor position in this view is updated, and\nit is reset to nil after usage."}, {Name: "listeners", Doc: "listeners is used for sending standard system events.\nChange is sent for BufferDone, BufferInsert, and BufferDelete."}, {Name: "autoSaving", Doc: "autoSaving is used in atomically safe way to protect autosaving"}, {Name: "markingUp", Doc: "markingUp indicates current markup operation in progress -- don't redo"}, {Name: "Changed", Doc: "Changed indicates if the text has been Changed (edited) relative to the\noriginal, since last EditDone"}, {Name: "NotSaved", Doc: "NotSaved indicates if the text has been changed (edited) relative to the\noriginal, since last Save"}, {Name: "fileModOK", Doc: "fileModOK have already asked about fact that file has changed since being\nopened, user is ok"}}})
