...
var theScreen = &screenImpl{
	...
	scrnData: make([]*oswin.ScreenData, 0),
}
...
type screenImpl struct {
	...
	winlist  []*windowImpl
	scrnData []*oswin.ScreenData
}

func (s *screenImpl) NewImage(size image.Point) (retBuf oswin.Image, retErr error) {
...
func (s *screenImpl) NewTexture(size image.Point) (oswin.Texture, error) {
...
func (s *screenImpl) NewWindow(opts *oswin.NewWindowOptions) (oswin.Window, error) {
...
func (s *screenImpl) NScreens() int {
	return len(s.scrnData)
}

func (s *screenImpl) ScreenData(scrN int) *oswin.ScreenData {
	sz := len(s.scrnData)
	if scrN < sz {
		return s.scrnData[scrN]
	}
	return nil
}

func (s *screenImpl) NWindows() int {
	return len(s.winlist)
}

func (s *screenImpl) Window(win int) oswin.Window {
	sz := len(s.winlist)
	if win < sz {
		return s.winlist[win]
	}
	return nil
}
