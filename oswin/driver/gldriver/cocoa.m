...
void pasteRead(char* typ, int len, NSPasteboard* pb) {
  NSDictionary *options = [NSDictionary dictionary];
  NSArray *classes;

  if (strncmp(typ, "text/plain", len) == 0) {
  ...
  if (strncmp(typ, "text/html", len) == 0) {
...
void clipRead(char* typ, int len) {
  ...
  pasteRead(typ, len, pb);
}


void clipWrite(char* data, int len, char* typ, int typlen) {
  NSPasteboard *pb = [NSPasteboard generalPasteboard];
  NSString *ns_clip;
  bool ret;
