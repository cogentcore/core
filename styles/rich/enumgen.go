...
var _FamilyValues = []Family{0, 1, 2, 3, 4, 5, 6, 7}

// FamilyN is the highest valid value for type Family, plus one.
const FamilyN Family = 8

var _FamilyValueMap = map[string]Family{`sans-serif`: 0, `serif`: 1, `monospace`: 2, `cursive`: 3, `fantasy`: 4, `maths`: 5, `emoji`: 6, `fangsong`: 7}
...
var _FamilyMap = map[Family]string{0: `sans-serif`, 1: `serif`, 2: `monospace`, 3: `cursive`, 4: `fantasy`, 5: `maths`, 6: `emoji`, 7: `fangsong`}
...
var _DecorationsValues = []Decorations{0, 1, 2, 3, 4, 5, 6}

// DecorationsN is the highest valid value for type Decorations, plus one.
const DecorationsN Decorations = 7

var _DecorationsValueMap = map[string]Decorations{`underline`: 0, `overline`: 1, `line-through`: 2, `dotted-underline`: 3, `fill-color`: 4, `stroke-color`: 5, `background`: 6}
...
var _DecorationsMap = map[Decorations]string{0: `underline`, 1: `overline`, 2: `line-through`, 3: `dotted-underline`, 4: `fill-color`, 5: `stroke-color`, 6: `background`}
...
var _SpecialsValues = []Specials{0, 1, 2, 3, 4}

// SpecialsN is the highest valid value for type Specials, plus one.
const SpecialsN Specials = 5

var _SpecialsValueMap = map[string]Specials{`nothing`: 0, `super`: 1, `sub`: 2, `link`: 3, `math`: 4}

var _SpecialsDescMap = map[Specials]string{0: `Nothing special.`, 1: `Super indicates super-scripted text.`, 2: `Sub indicates sub-scripted text.`, 3: `Link indicates a hyperlink, which must be formatted through the regular font styling properties, but this is used for functional interaction with the link element.`, 4: `Math indicates a LaTeX formatted math sequence.`}

var _SpecialsMap = map[Specials]string{0: `nothing`, 1: `super`, 2: `sub`, 3: `link`, 4: `math`}
