...
func (sh *Shell) TranspileLineTokens(ln string) Tokens {
	...
	case t0in == 0: // exec must be IDENT
		...
		return sh.TranspileGo(toks)
	case t0in > 0 && n > t0in && (t1in > 0 || t1pn > 0 || toks[t1idx].Tok == token.SUB || toks[t1idx].Tok == token.DEC || toks[t1idx].Tok == token.GTR || toks[t1idx].Tok == token.SHR || toks[t1idx].Tok == token.LSS || toks[t1idx].Tok == token.STRING || toks[t1idx].Tok == token.LBRACE || toks[t1idx].Tok == token.ILLEGAL):
