...
type Buffer struct {
	...
	// wgpu representation of the buffer
...
func (mb *Buffer) AllocHost(dev vk.Device, bsz int) bool {
	if bsz == mb.Size {
		return false
	}
	mb.Free(dev)
...
func (mb *Buffer) AllocDev(dev vk.Device) {
...
func (mb *Buffer) Free(dev vk.Device) {
	if mb.Size == 0 {
		return
	}
	if mb.Type != TextureBuff {
...
const (
	// IndexBuffer is a buffer holding Vertex and Index values
	IndexBuffer BufferTypes = iota
	...
	// TextureBuff holds Images / Textures. Not actually a buffer
...
// BuffUsages maps BufferTypes into buffer usage flags
var BuffUsages = map[BufferTypes]vk.BufferUsageFlagBits{
	IndexBuffer:   vk.BufferUsageVertexBufferBit | vk.BufferUsageIndexBufferBit,
	UniformBuffer: vk.BufferUsageUniformBuffererBit | vk.BufferUsageUniformTexelBufferBit,
	StorageBuffer: vk.BufferUsageStorageBuffererBit | vk.BufferUsageStorageTexelBufferBit,
...
func AllocBuffMem(gp *GPU, dev vk.Device, buffer vk.Buffer, properties vk.MemoryPropertyFlagBits) vk.DeviceMemory {
	...
	if !ok {
		log.Println("WebGPU warning: failed to find required memory type")
	}
...
func MapMemory(dev vk.Device, mem vk.DeviceMemory, size int) unsafe.Pointer {
	...
	if IsError(ret) {
		log.Printf("WebGPU MapMemory warning: failed to map device memory for data (len=%d)", size)
		return nil
	}
	return buffPtr
}
...
func MapMemoryAll(dev vk.Device, mem vk.DeviceMemory) unsafe.Pointer {
	...
	if IsError(ret) {
		log.Printf("WebGPU MapMemory warning: failed to map device memory for data")
		return nil
	}
	return buffPtr
}
